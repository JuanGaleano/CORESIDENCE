---
title: "TEST: ARG_1970_IPUMS"
author: "CORESIDENCE PROJECT"
abstract: "**Abstract: **This document provides a summary description of variables included in the samples downloaded from IPUMS to build the CORESIDENCE database. If you need to report any mistake, please contact jgaleano@ced.uab.es. You can access the source r-code to render this document at: [GitHub CORESIDENCE](https://github.com/JuanGaleano/CORESIDENCE/blob/main/TEST_IPUMS.qmd)"
mainfont: Arial
monofont: Arial
format: pdf
editor: visual
...
load("G:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_1_Input Data\\CENSUS_IPUMS\\ARG_1970_IPUMS.Rdata")
...
## COUNTRY: `r toupper(as.character(as_factor(unique(df$COUNTRY))))` | YEAR: `r as.character(as_factor(unique(df$YEAR)))` \| SAMPLE SIZE: `r nrow(df)` CASES
...
df_all_NAs_f<-data.frame(Var=row.names(df_all_NAs),
...
### 3.3 Population pyramid (AGE x SEX x RELATE)
...
### 6. NFAMS, NCOUPLES, NMOTHERS, NFATHERS, NCHILD, NCHLT5
...
### 7. AGE_HEAD, AGE_MOM, AGE_POP, AGE_SP
...
kbl(ELDCH_AGE2, booktabs = T)%>%kable_styling(full_width = T,position = "center") %>%column_spec(1, width = "30em")
```
...
kbl(YNGCH_AGE2, booktabs = T)%>%kable_styling(full_width = T,position = "center") %>% column_spec(1, width = "30em")
```
...
### 11. EDUCATION: EDATTAIN, EDATTAIND
...
### 11.3 EDATTAIN_HEAD, EDATTAIN_MOM, EDATTAIN_POP, EDATTAIN_SP
...
### 11.4 Frequencies EDATTAIN_HEAD
...
### 12. LABOUR FORCE: EMPSTAT, EMPSTATD
...
### 12.3 EMPSTAT_HEAD, EMPSTAT_MOM, EMPSTAT_POP, EMPSTAT_SP
...
### 12.12 STATUS OF ECONOMICALLY ACTIVE PERSON: CLASSWK, CLASSWKD
...
### 13. OWNERSHIP, OWNERSHIPD, HHTYPE
...
### 14. MARITAL STATUS: MARST, MARSTD, CONSENS
...
### 15.3 Number of unique codes in GEOLEV2
...
kbl(GQ, booktabs = T)%>%kable_styling(full_width = T,position = "center") %>%
column_spec(1, width = "30em")

```

### 17.2 Frequencies UNREL
...
kbl(UNREL, booktabs = T)%>%kable_styling(full_width = T,position = "center") %>%
column_spec(1, width = "30em")
...
### 18.5 AGE x SEX x EDATTAIN (Graph)
