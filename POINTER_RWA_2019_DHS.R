...
load("I:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_4_CORESIDENCE_DATABASE\\CORESIDENCE_LIVING_ARRANGEMENTS\\CORE_FILES\\DHS_TO_DB/RWA_2019-20_DHS.RData")
...
FUN_POPLOC_DHS <- function(df) {
  ...
  df <- df |>
    mutate(
      max_HV105_5700 = max(HV105[RELATED == 5700], na.rm = TRUE), # Calculate max HV105 for RELATED == 5900 once for efficiency
      POPLOC2 = ifelse(!is.na(POPLOC2), POPLOC2,
                       ifelse(RELATED == 5700 & 
                                (!any(RELATED == 5700 & HV104 == 1) | 
                                   sum(RELATED == 5700) == 1 | 
                                   HV105 == max_HV105_5700 | 
                                   HV105 < max_HV105_5700), 0, NA))
    ) %>%
    select(-max_HV105_5700) # Remove auxiliary column after use
...
save(df, file="I:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_4_CORESIDENCE_DATABASE\\CORESIDENCE_LIVING_ARRANGEMENTS\\CORE_FILES\\DHS_TO_GELAI\\RWA_2019_DHS_GELAI.Rda")
