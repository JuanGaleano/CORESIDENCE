...
abstract: "**Abstract: **This document provides a summary description of variables included in the samples downloaded from IPUMS to build the CORESIDENCE database. You can access the source code to render this document at: [GitHub CORESIDENCE](https://github.com/JuanGaleano/CORESIDENCE)"
mainfont: Arial
monofont: Arial
format: pdf
editor: visual
...
allNas<- data.frame(Variables=df %>% keep(~all(is.na(.x))) %>% names, 
                    Description=c(attributes(df$FORMTYPE)$var_desc,                  attributes(df$URBAN)$var_desc,
              attributes(df$RESIDENT)$var_desc,                  attributes(df$BIRTHYR)$var_desc,
