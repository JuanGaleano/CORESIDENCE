...
load("A3_CORESIDENCE_IPUMS_list.RData")


# CHECK OVER RELATE AND RELATED CATEGORIES ######
RELATE <-lapply(CORE_LIST, function(df) {
  
  df<-df%>%
    select(RELATED)%>%
    mutate(cat=as_factor(RELATED))%>%
    mutate(RELATEb<-as.character(RELATED))
  
  df<-df[,c(2,3)]
  colnames(df)[2]<-"RELATE"
  df$cat<-as.character(df$cat)
  df<-as.data.frame(df)
  df
})

class(agrr_RELATE$RELATE)
sort(unique(agrr_RELATE$RELATE))

# WHICH IS THE PROPORTION OF THE POPULATION LIVING IN PRIVATE HOUSEHOLDS FOR THE MOST RECENT DATA? ######
POP_IN_HH<-lapply(CORE_LIST, function(df) {
  df<-df %>%
    filter(DECADE>=2010)%>%
    mutate(hper=1)%>%
    group_by(DECADE,GQ=as_factor(GQ)) %>% 
    summarise(n=sum(hper*PWEIGHT_A))%>% 
    mutate(n_rel = round(n / sum(n)*100,2))%>% 
    filter(GQ=="Households")
  
});respuesta<-do.call("rbind", POP_IN_HH);mean(respuesta$n_rel)

POP_IN_SAMPLES<-lapply(CORE_LIST, function(df) {
  df<-nrow(df)
});respuesta<-do.call("rbind", POP_IN_SAMPLES);sum(respuesta)



# EXAMPLE OVER ONE SAMPLE #####

exemple<-as_tibble(CORE_LIST[["ARG_1980_IPUMS.RData.df"]])
head(exemple)
sum(exemple$PWEIGHT)
VALID_GQ<-c(10,99)
...
df2<-df %>% #### STEP2 #####
  group_by(SAMPLE,CNTRY,YEAR,SERIAL,CONTINENT,CNTRY_ISO,SOURCE,CENSUS_ROUND,DECADE,FIVE_YEAR,CONTINENT2,WEIGHT_POP,WEIGHT_A,WEIGHT_A2,WEIGHT_A3,WEIGHT_A4,WEIGHT_A5, WEIGHT_AM,WEIGHT_AF)%>%
  summarise(hh=1,
            hh_m=sum(M_headed),
            hh_f=sum(F_headed),
            hh_per1=n(),
...
df<-as.data.frame(df) #### STEP 3 ######
df2<-as.data.frame(df2)
df<- data.frame(df,df2[match(df[,"SERIAL"],
                             df2[,"SERIAL"]),c("family_type","family_type2")])

df<-df %>%
  mutate(WEIGHT_FT10= sum(PWEIGHT[RELATE==1&family_type==10]),
...
df2<-df %>% ##### STEP 4 #######
  group_by(SAMPLE,CNTRY,YEAR,SERIAL,CONTINENT,CNTRY_ISO,
           ...
           WEIGHT_A5, WEIGHT_AM,WEIGHT_AF,WEIGHT_FT10,WEIGHT_FT20,
...
average_size_HH2<-df2 %>% #### STEP 5 #######
  ...
  summarise(WPOP=labelled(x=round(unique(WEIGHT_POP),0),label="Weighted population in sample"),
            ...
            HS12=labelled(x=sum(hh_child5 != 0)/sum(hh),label="Proportion of households with at least one person below 6"),
            HS13=labelled(x=sum(hh_elderly65 != 0)/sum(hh),label="Proportion of households with at least one person over 64"),
            
            HS14=labelled(x=sum(hh_per)/unique(WEIGHT_A),label="Average household size"),
            HS15=labelled(x=sum(hh_child5)/unique(WEIGHT_A),label="Average number of under 5 children in the household"),
            ...
            HH39=labelled(x=sum(hh_childs[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of children of male-headed households"),
            HH40=labelled(x=sum(hh_childs[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of children of female-headed households"),
            ...
            HH43=labelled(x=sum(hh_spouses[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of spouses of male-headed households"),
            HH44=labelled(x=sum(hh_spouses[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of spouses of female-headed households"),
            HH45=labelled(x=sum(hh_childs[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of childs of male-headed households"),
            HH46=labelled(x=sum(hh_childs[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of childs of female-headed households"),
            HH47=labelled(x=sum(hh_other_relatives[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of other relatives of male-headed households"),
            HH48=labelled(x=sum(hh_other_relatives[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of other relatives of female-headed households"),
            HH49=labelled(x=sum(hh_non_relatives[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of other non relatives of male-headed households"),
            HH50=labelled(x=sum(hh_non_relatives[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of other non relatives of female-headed households"),
            
            HH51=labelled(x=sum(hh_elderly65[hh_M_headed!=0])/unique(WEIGHT_AM),label="Average number of elderly people (aged >65) of male-headed households"),
            HH52=labelled(x=sum(hh_elderly65[hh_F_headed!=0])/unique(WEIGHT_AF),label="Average number of elderly people (aged >65) of female-headed households"),
...
clipr::write_clip(average_size_HH2)
...
# STEP 4 LIST ####
rm(PASO1)
PASO3<-lapply(PASO3, function(df) { 
...
# STEP 6 LIST ######
agrr_database_list<-lapply(PASO4, function(df) {
...
# STEP 7 GET THE RESULTS OUT OF THE LIST AND SAVE #####
...
save(agrr_database_ipums, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_IPUMS.RData")
save(agrr_database_ipums_long, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_IPUMS_long.RData")


agrr_database_ipums_long$CNTRY<-as_factor(agrr_database_ipums_long$CNTRY)
library(writexl)
write_xlsx(agrr_database_ipums_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_IPUMS_longnew.xlsx")
...
library(haven)
PGPR71FL <- read_sav("new_data_March2023/PGPR71FL.SAV")

df$year_control<-2017

save(df, file="PNG_2016-18_DHS.RData")
#########
...
# CHECK NAs IN VARIABLE ######
check<-lapply(sample_DHS_LIST, function(df) {
  df<-if("HV105" %notin% colnames(df))
  {"Variable not in sample";
  } else {
    if(all(is.na(df$HV105))==TRUE)
    {"100% NAs";
    } else {"OK"             
    }}
  df
...
var<-as.data.frame(t(as.data.frame(check)))

var$COUNTRY<-substr(row.names(var),1,3)
...
POP_IN_SAMPLES_dhs<-lapply(sample_DHS_LIST, function(df) {
  df<-nrow(df)
});respuesta_dhs<-do.call("rbind", POP_IN_SAMPLES_dhs);sum(respuesta_dhs)
...
a$SOURCE<-"DHS"
a$SAMPLE<-with(a,paste(country_control, year_control, SOURCE, sep="_"))
a$PWEIGHT_A<-(a$HV005/1000000)
a$hh<-1
a$AGE<-as.numeric(a$HV105)
a$YEAR<-as.numeric(substr(a$SAMPLE,5,8))
a$AGE[is.na(a$AGE)] <- 999

a$AGE2<-with(a,  ifelse(AGE==999, "999",
                          ifelse(AGE<=4, "0-4",
                                 ifelse(AGE<=9, "5-9", 
                                        ifelse(AGE<=14, "10-14",
                                               ifelse(AGE<=19, "15-19",
...
a$CENSUS_ROUND<-with(a,ifelse((YEAR>=1956 & YEAR <=1965), 1960,
...
a$DECADE<-with(a,ifelse((YEAR>=1951 & YEAR <=1960), 1950,
...
a$FIVE_YEAR<-with(a,ifelse((YEAR>=1951 & YEAR <=1955), 1951,
...
a$YEAR2<-as.numeric(as.character(a$year_control))

levels(a$HV101)
class(a$HV101)

a$HV101b<-as.character(as_factor(a$HV101))

class(a$HV101b)


a$HV101b[is.na(a$HV101b)] <- "Missing"

##### HARMONIZATION CATEGORIES RELATED ######
a<-a%>%
  mutate(HV101c=case_when(HV101b == "Head" ~ "Head", #
                          ...
                          HV101b == "Step-son/step-daughter" ~ "Son/daughter-in-law",
                          HV101b == "Step child" ~ "Son/daughter-in-law",
                          HV101b == "Stepchild" ~ "Son/daughter-in-law",
                          HV101b == "Stepson/daughter" ~ "Son/daughter-in-law",
                          ...
                          HV101b == "Adopted child" ~ "Adopted/foster child",#
                          ...
                          HV101b == "Adopted/foster/stepchild" ~ "Adopted/foster child",
                          HV101b == "Foster" ~ "Adopted/foster child",
                          ...
                          HV101b == "Nephew/niece in law" ~ "Niece/nephew",
                          HV101b == "Niece/ nephew by blood" ~ "Niece/nephew",
                          HV101b == "Niece/ nephew by marriage" ~ "Niece/nephew",
                          HV101b == "Niece/nehew by blood" ~ "Niece/nephew",
                          ...
                          HV101b == "Great grandson/granddaughter" ~ "Great Grandparent"))
...
a<-a%>%
  mutate(RELATE=case_when(HV101c == "Head" ~ 1,#
         ...
         RELATED=case_when(HV101c == "Head" ~ 1000,#
                           ...
                           HV101c == "Wife or husband" ~ 2000,#
                           HV101c == "Co-spouse" ~ 2000,#
                           HV101c == "Cohabiting partner" ~ 2200,#
                           
                           HV101c == "Son/daughter" ~ 3000,#
                           HV101c == "Son/daughter-in-law" ~ 3400,#
                           HV101c == "Adopted/foster child" ~ 3200,#
                           ...
                           HV101c == "Mother not listed" ~ 4900,#
                           HV101c == "Grandparent" ~ 4500,
                           HV101c == "Great Grandparent" ~ 4510,
                           HV101c == "Other relative" ~ 4000,#
...
a$RELATED<-labelled(x=a$RELATED,
                     labels=c("Head" = 1000,#
                              "Spouse/partner" = 2000,#
...
a<-as.data.frame(a)
a<- data.frame(a,
                regionw[match(a[,"country_control"],
                              regionw[,"ISO3"]),c("REGIONW")])
colnames(a)[grep(names(a %>% select(last_col())), colnames(a))]<-"REGIONW"
...
# CHECK OVER RELATE AND RELATED CATEGORIES ######
HV101 <-lapply(sample_DHS_LIST, function(df) {
 
  df<-df%>%
    select(HV101)%>%
    mutate(cat=as_factor(HV101))%>%
    mutate(HV101b<-as.character(HV101))
...
agrr_HV101 <- do.call("rbind", HV101)
agrr_HV101$cat2<-paste(agrr_HV101$cat,agrr_HV101$HV101,sep="-")

df2<-data.frame(var=unique(agrr_HV101$cat))
df2<-df2%>%
  arrange(var)


HV101df <-lapply(HV101, function(df) {
...
samples <- as.list(names(HV101df))

final<-Map(cbind, HV101df, sample = samples)

samples_all <- do.call("rbind", final)

dfp1<-samples_all%>%
filter(cat=="Parent / parent-in-law");dfp1;HV101df[[dfp1$sample]]

dfp2<-samples_all%>%
  filter(cat=="Parent/ parent-in-law");dfp2;HV101df[[dfp2$sample[1]]];HV101df[[dfp2$sample[2]]]

dfp3<-samples_all%>%
  filter(cat=="Parent/parent-in-law");dfp3;HV101df[[dfp3$sample[1]]];HV101df[[dfp3$sample[2]]]

dfp4<-samples_all%>%
  filter(cat=="Parents,p. in-law");dfp4;HV101df[[dfp4$sample[1]]]

dfp5<-samples_all%>%
  filter(cat=="Parents/ parent-in-law");dfp5;HV101df[[dfp5$sample[1]]];HV101df[[dfp5$sample[2]]]

parents<-rbind(dfp1,dfp2,dfp3,dfp4);parents

dfp6<-samples_all%>%
  filter(cat=="Co-spouse");dfp6

dfp7<-samples_all%>%
  filter(cat=="Uncle/Aunt/Other relative");dfp7





#aver<-HV101[["AFG_2015_DHS.RData.df"]]
#aver$cat<-as.character(aver$cat)
...
# STEP 1 LIST ######
PASO1_dhs<-lapply(CORE_DHS_LIST, function(df) {
...
PASO2<-lapply(PASO1_dhs, function(df) {  
  df<-df%>%
    group_by(HHID)%>%
    summarise(hh_per1=n(),
...
PASO3<- map2(PASO1_dhs, PASO2, ~left_join(.x, .y, by = 'HHID'))
...
PASO3<-lapply(PASO3, function(df) { 
  ...
  df<-df %>%
    mutate(WEIGHT_FT10= sum(PWEIGHT_A[RELATE==1&family_type==10]),
...
PASO4<-lapply(PASO3, function(df) {
  df<-df %>% ##### STEP 4 #######
  group_by(SAMPLE,country_control,YEAR,HHID,CONTINENT,SOURCE,
...
# STEP 6 LIST ######
agrr_database_list_dhs<-lapply(PASO4, function(df) {
...
# STEP 7 GET THE RESULTS OUT OF THE LIST ADD COUNTRY NAME AND SAVE #####

agrr_database_dhs <- do.call("rbind", agrr_database_list_dhs)
...
save(agrr_database_dhs, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_DHS.RData")
...
library(writexl)
write_xlsx(agrr_database_dhs_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_DHS_long.xlsx")
...
POP_IN_SAMPLES_LFS<-lapply(sample_LFS_LIST, function(df) {
  df<-nrow(df)
});respuesta_lfs<-do.call("rbind", POP_IN_SAMPLES_LFS);sum(respuesta_lfs)

# CHECK OVER RELATE AND RELATED CATEGORIES ######
HHLINK <-lapply(sample_LFS_LIST, function(df) {
  
  df<-df%>%
    select(COUNTRY )%>%
    mutate(COUNTRY = as.factor(COUNTRY ))%>%
    mutate(cat=fct_explicit_na(COUNTRY , na_level = "Missing"))%>%
    mutate(HV101b<-as.character(COUNTRY ))
...
agrr_HV101 <- do.call("rbind", HHLINK)

aver<-HHLINK[["DEU_2015_LFS.RData.df"]]

#aver$cat<-as.character(aver$cat)

df2<-data.frame(var=levels(agrr_HV101$cat))
...
# STEP 1 LIST ######
PASO1_lfs<-lapply(CORE_LFS_LIST, function(df) {
...
PASO2<-lapply(PASO1_lfs, function(df) {  
  df<-df%>%
    group_by(hhid)%>%
    summarise(hh_per1=n(),
...
PASO3<- map2(PASO1_lfs, PASO2, ~left_join(.x, .y, by = 'hhid'))
...
PASO3<-lapply(PASO3, function(df) { 
  ...
  df<-df %>%
    mutate(WEIGHT_FT10= sum(COEFF[RELATE==1&family_type==10]),
...
# STEP 6 LIST ######
agrr_database_list_lfs<-lapply(PASO4, function(df) {
...
save(agrr_database_lfs, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_LFS.RData")

#### para los que rescatamos 
average_size_HH2<-as.data.frame(average_size_HH2)
average_size_HH2<- data.frame(average_size_HH2,
                               regionw[match(average_size_HH2[,"ISO3"],
                                             regionw[,"ISO3"]),c("EU Member States")])

colnames(average_size_HH2)[grep(names(average_size_HH2 %>% select(last_col())), colnames(average_size_HH2))]<-"CNTRY"

save(average_size_HH2, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\POL_2006_LFS.RData")




agrr_database_lfs_long<-agrr_database_lfs%>%
  pivot_longer(c(WPOP:HH56), names_to = "indicator", values_to = "value") 


library(writexl)

write_xlsx(agrr_database_lfs_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_LFS_long.xlsx")
...
#######
POP_IN_SAMPLES_SILC<-lapply(samples_SILC_LIST2, function(df) {
  df<-nrow(df)
});respuesta_silc<-do.call("rbind", POP_IN_SAMPLES_SILC);sum(respuesta_silc)
...
# STEP 1 LIST ######
PASO1_silc<-lapply(CORE_SILC_LIST, function(df) {
...
PASO2<-lapply(PASO1_silc, function(df) {  
  df<-df%>%
    group_by(HHID)%>%
    summarise(hh_per1=n(),
...
PASO3<- map2(PASO1_silc, PASO2, ~left_join(.x, .y, by = 'HHID'))
...
PASO3<-lapply(PASO3, function(df) { 
  ...
  df<-df %>%
    mutate(WEIGHT_FT10= sum(RB050[RELATE==1&family_type==10]),
...
# STEP 6 LIST ######
agrr_database_list_silc<-lapply(PASO4, function(df) {
...
save(agrr_database_silc, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_SILC.RData")
...
write_xlsx(agrr_database_silc_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_SILC_long.xlsx")
...
#######
POP_IN_SAMPLES_K<-lapply(samples_KOREA_LIST, function(df) {
  df<-nrow(df)
});respuesta_K<-do.call("rbind", POP_IN_SAMPLES_K);sum(respuesta_K)
...
CORE_KOREA_LIST<-lapply(samples_KOREA_LIST, function(df) {
  df$COUNTRY<-"KR"
...
# STEP 1 LIST ######
PASO1_kor<-lapply(CORE_KOREA_LIST, function(df) {
...
PASO2<-lapply(PASO1_kor, function(df) {  
  df<-df%>%
    group_by(hhnum)%>%
    summarise(hh_per1=n(),
              hh_parents=sum(parents*PWEIGHT_A),
  ...
  df<-df%>%select(hhnum,family_type,family_type2)             
  df
})

# STEP 3 LIST ####

PASO3<- map2(PASO1_kor, PASO2, ~left_join(.x, .y, by = 'hhnum'))
...
# STEP 6 LIST ######
agrr_database_list_korea<-lapply(PASO4, function(df) {
...
# STEP 7_ GET THE RESULTS OUT OF THE LIST ADD COUNTRY NAME AND SAVE #####

agrr_database_korea <- do.call("rbind", agrr_database_list_korea)
...
clipr::write_clip(agrr_database_korea)
save(agrr_database_korea, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_KOREA.RData")


agrr_database_korea_long<-agrr_database_korea%>%
  pivot_longer(c(WPOP:HH56), names_to = "indicator", values_to = "value") 


library(writexl)

write_xlsx(agrr_database_korea_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_KOREA_long.xlsx")
...
# STEP 1 LIST ######
PASO1_AUS<-lapply(CORE_AUS_LIST, function(df) {
  df<-df %>%
    #filter(GQ %in% VALID_GQ) %>% #filtrem només els private households
    add_count(hhrhid)%>%
    mutate(child5=ifelse(AGE<=4,1,0),
...
# STEP 2 LIST ####### 
PASO2<-lapply(PASO1_AUS, function(df) {  
  df<-df%>%
    group_by(hhrhid)%>%
    summarise(hh_per1=n(),
              hh_parents=sum(parents*PWEIGHT_A),
...
PASO3<- map2(PASO1_AUS, PASO2, ~left_join(.x, .y, by = 'hhrhid'))
...
# STEP 6 LIST ######
agrr_database_list_aus<-lapply(PASO4, function(df) {
...
save(agrr_database_aus, file="G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_AUS.RData")


agrr_database_aus_long<-agrr_database_aus%>%
  pivot_longer(c(WPOP:HH56), names_to = "indicator", values_to = "value") 

library(writexl)

write_xlsx(agrr_database_aus_long,"G:\\Shared drives\\CORESIDENCE\\TEAM FOLDERS\\Juan Galeano\\AGGR_INDICATORS_AUSTRALIA_long.xlsx")
...
load("G:/Shared drives/CORESIDENCE/TEAM FOLDERS/Juan Galeano/AGGR_INDICATORS_AUS.RData")
...
CORESIDENCE_AGG<-rbind(agrr_database_ipums,agrr_database_dhs,agrr_database_lfs,agrr_database_silc,agrr_database_korea,agrr_database_aus)
...
library(writexl)
write_xlsx(CORESIDENCE_AGG,"G:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_4_CORESIDENCE_DATABASE\\A0_CORESIDENCE_AGG_new.xlsx")
save(CORESIDENCE_AGG,file="G:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_4_CORESIDENCE_DATABASE\\A0_CORESIDENCE_AGG_new.RData")


CORESIDENCE_AGG_L<-CORESIDENCE_AGG%>% 
  #select(-HDICODE)%>% 
  pivot_longer(cols=c(HS01:HH56, P1:D14),names_to = "Indicator", values_to = "value" )

write_xlsx(CORESIDENCE_AGG_L,"G:\\Shared drives\\CORESIDENCE\\WP2_DATA\\2_4_CORESIDENCE_DATABASE\\A0_CORESIDENCE_AGG_L_new.xlsx")
